...
If you require all protected area data for a country, including areas represented as point localities, you will need to fetch the global dataset (approx. 1GB) and filter out protected areas manually. Below is some example code to achieve this, but note that it can take over 20 minutes to complete. This code is provided to serve as a reference and you do not need to run it to complete this tutorial.

```{r}
# # fetch global data
# global_raw_data <- wdpa_fetch("global", wait = TRUE)
#
# # subset data to include only Malta
# mlt_raw_pa_data <- global_raw_data %>% filter(ISO == "MLT")
```
